...
func update_countdown_label_text() -> void:
	...
	if show_time_units:
		str_days    = str("%02d" % time_left.days) + "d "    if time_left.days    > 0 else ""
		str_hours   = str("%02d" % time_left.hours) + "h "   if time_left.hours   > 0 else ""
		str_minutes = str("%02d" % time_left.minutes) + "m " if time_left.minutes > 0 else ""
		str_seconds = str("%02d" % time_left.seconds) + "s"  if time_left.seconds > 0 else ""
	else:
...
func _on_TitleLabel_pressed():
	OS.shell_open(jam_link);
